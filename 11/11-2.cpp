...
struct robo {
	prog *brain;
	int x, y, dir, painted;
	robo(char* file) : brain(new prog(file, true, 2)), x(0), y(0), dir(0), painted(0) {}

	void run() {
		while (brain->canRun()) {
			read();
			brain->exec();
			int a = brain->bufget(0), b = brain->bufget(1);
	...
	void read() {
		brain->bufset(2, mapa[x][y]);
	}

	void walk() {
		x += (dir == 1) - (dir == 3);
		y += (dir == 0) - (dir == 2);
		xb = {min(xb.first, x), max(xb.second, x)};
		yb = {min(yb.first, y), max(yb.second, y)};
	}
};

int main(int argc, char** argv) {
	if (argc == 1) cout << "input file needed" << endl, exit(0);

	robo r(argv[1]);
	mapa[0][0] = 1;
	r.run();
	for (int i = yb.second;i >= yb.first;i--) {
		for (int j = xb.first;j <= xb.second + 1;j++) {
			cout << (mapa[j][i] ? '#' : ' ');
		}
		cout << endl;
	}
